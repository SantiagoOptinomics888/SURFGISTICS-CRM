  ...
  },
  {
    ...
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
        ...
        <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 15.75h7.5M8.25 18h4.5M8.25 12h7.5" />
      </svg>
    ),
  },
  {
    ...
    <aside className="w-60 min-h-screen bg-[#0F172A] flex flex-col">
      ...
      <div className="px-5 py-5 border-b border-[#1E293B]">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded bg-[#0369A1] flex items-center justify-center flex-shrink-0">
            <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5a17.92 17.92 0 0 1-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418" />
            </svg>
          </div>
          <span className="text-white font-semibold text-sm tracking-wide">Surfgistics</span>
        </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {links.map(({ href, label, icon }) => {
          ...
          return (
            ...
            >
              <span className={cn(isActive ? "text-[#0EA5E9]" : "text-[#64748B] group-hover:text-[#94A3B8]")}>
                {icon}
              </span>
              {label}
            </Link>
      ...
      <div className="px-3 py-4 border-t border-[#1E293B]">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-md">
          <div className="w-7 h-7 rounded-full bg-[#0369A1] flex items-center justify-center flex-shrink-0">
            <span className="text-white text-xs font-semibold">{initials}</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[#CBD5E1] text-xs font-medium truncate">{user?.email}</p>
            {user?.importer_account && (
              <p className="text-[#475569] text-xs truncate">{user.importer_account}</p>
            )}
          </div>
        </div>
        <button
          onClick={handleLogout}
          className="mt-1 w-full flex items-center gap-3 px-3 py-2 rounded-md text-[#64748B] hover:text-[#94A3B8] hover:bg-[#1E293B] text-sm transition-colors duration-150 cursor-pointer"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
